...
int getDistance(string key, InstructionServer &server)
{
    if (key == "north")
...
bool isWall(string key, InstructionServer &server)
{
    if (getDistance(key, server) > 20)
        return false;
    else
        return true;
}
...
Tile *followPath(stack<string> &path, Tile *tile, vector<int> &pos, InstructionServer &server)
{
    while (!path.empty())
    {
        tile = move(tile, path.top(), pos, xMaze, yMaze, rDirection, server);
...
void explore(Tile *tile, vector<Tile *> &unvisited, InstructionServer &server)
{
    // Tile *startTile = tile;
