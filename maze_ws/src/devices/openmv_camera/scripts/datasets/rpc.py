...
class rpc:
    ...
    def _set_packet(self, magic_value, payload=bytes()):  # private
        ...
        new_payload[-2:] = struct.pack("<H",
                                       self.__crc_16(new_payload, len(payload) + 2))
        return new_payload
    ...
    def stream_reader(self, call_back, queue_depth=1, read_timeout_ms=5000):  # public
        try:
            self._stream_put_bytes(self._set_packet(
                0xEDF6, struct.pack("<I", queue_depth)), 1000)
        except OSError:
            return
        tx_lfsr = 255
        while True:
    ...
    def stream_writer(self, call_back, write_timeout_ms=5000):  # public
        ...
        if magic != 0xEDF6 and crc != self.__crc_16(packet, len(packet) - 2):
            return
        queue_depth = max(min(struct.unpack(
            "<I", packet[2:-2])[0], self._stream_writer_queue_depth_max), 1)
        rx_lfsr = 255
        credits = queue_depth
        while True:
            ...
            if credits > 0:
                ...
                try:
                    self._stream_put_bytes(self._set_packet(
                        0x542E, struct.pack("<I", len(data))), 1000)
                except OSError:
                    return
                try:
...
class rpc_master(rpc):
    ...
    def __put_command(self, command, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._zero(self.__in_command_header_buf[0], len(
                self.__in_command_header_buf[0]))
            self._zero(self.__in_command_data_buf[0], len(
                self.__in_command_data_buf[0]))
            self._flush()
            ...
            self._put_short_timeout = min(
                (self._put_short_timeout * 4) // 3, timeout)
            self._get_short_timeout = min(
                (self._get_short_timeout * 4) // 3, timeout)
        return False
    ...
    def __get_result(self, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            gc.collect()  # Avoid collection during the transfer.
            self._zero(self.__in_result_header_buf[0], len(
                self.__in_result_header_buf[0]))
            self._flush()
            self.put_bytes(self.__out_result_header_ack,
                           self._put_short_timeout)
            packet = self._get_packet(
                self._RESULT_HEADER_PACKET_MAGIC, self.__in_result_header_buf, self._get_short_timeout)
            if packet is not None:
...
class rpc_slave(rpc):
    ...
    def __get_command(self, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._flush()
            packet = self._get_packet(
                self._COMMAND_HEADER_PACKET_MAGIC, self.__in_command_header_buf, self._get_short_timeout)
            if packet is not None:
    ...
    def __put_result(self, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            gc.collect()  # Avoid collection during the transfer.
            self._zero(self.__in_response_header_buf[0], len(
                self.__in_response_header_buf[0]))
            self._zero(self.__in_response_data_buf[0], len(
                self.__in_response_data_buf[0]))
            self._flush()
...
class rpc_uart_master(rpc_master):
    ...
    def __get_bytes(self, buff):
        ...
        l = len(buff)
        while l:
            # Starts a new timeout per call.
            data = self.__ser.read(min(l, 1024))
            data_len = len(data)
            if not data_len:
                return None
    ...
    def get_bytes(self, buff, timeout_ms):  # protected
        if int(self.__ser.timeout * 100) != 1:
            # Changing this causes control transfers.
...
class rpc_network_master(rpc_master):
    ...
    def __valid_udp_socket(self):  # private
        if self.__udp__socket is None:
            try:
                self.__udp__socket = socket.socket(
...
class rpc_network_slave(rpc_slave):
    ...
    def __valid_tcp_socket(self):  # private
        if self.__tcp__socket is None:
            try:
                self.__tcp__socket = socket.socket(
        ...
        for i in range(0, len(view), 8):
            try:
                self.__can.writeWait(
                    Frame(id_=self.__message_id, data=view[i:i+8]), timeout=self._put_short_timeout)
            except canlib.CanError:
                break
